...
final class GlassesServer: ObservableObject {
    ...
    func sendLocation(lat: Double, lng: Double, bearing: Float, speed: Float, accuracy: Float, speedLimit: Int = -1, distToNext: Double = -1) {
        ...
        RokidMobileSDK.vui.sendMessage(topic: "location", text: String(describing: lat), to: dev)
    }

    func sendStep(instruction: String, maneuver: String, distance: Double) {
        guard let dev = activeDevice else { return }
        RokidMobileSDK.vui.sendMessage(topic: "step", text: String(describing: instruction), to: dev)
    }

    func sendRoute(waypoints: [[String: Double]], totalDistance: Double, totalDuration: Double) {
        guard let dev = activeDevice else { return }
        RokidMobileSDK.vui.sendMessage(topic: "route", text: String(describing: String), to: dev)
    }
